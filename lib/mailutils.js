...
function decodeRFC2407(text) {
    ...
    //stolen
    let encodedWordRegExp = /\=\?([^\?]+)\?([QB])\?([^\?]*)\?=/gi;
    let itemsToReplace = text.match(encodedWordRegExp);
    ...
    let decodedPhrase = text.replace(encodedWordRegExp, function(encodedWord, charset, encoding, encodedText, index) {

        if (encoding.toLowerCase()==='b') {
            if (charset.toLowerCase()==='iso-8859-1') {
                return (new Buffer(encodedText, 'base64').toString('binary'));
            } else {
                return (new Buffer(encodedText, 'base64').toString('utf-8'));
            }
        } else {   //otherwise the encoding is 'q'
            let removedUnderscores = encodedText.replace(/_/g, ' ');
            let finalWord = '';

            for (let i = 0; i < removedUnderscores.length; i++) {
                if (encodedText.charAt(i) === '=') {
                    //if we hit an = we need the next values
                    let code = removedUnderscores.slice(i + 1, i + 3);   //no multi byte support yet
...
function stripComments(text) {
   ...
   */
    ...
    if (potentialComments != null) {
        for (let i=0; i< potentialComments.length; i++) {
            let trimmedWord = potentialComments[i].trim();
...
function writeCB(err, written, buffer) {
    if (err) {
        console.log('Error occurred writing data: ' + err);
    } else {
        //console.log(buffer);
    }
}
...
function processDataLine(line, prevHeaderLine, fd) {
    ...
    if (startsWithKnownHeader(line) && startsWithKnownHeader(prevHeaderLine)) {
        fs.write(fd, stripComments(decodeRFC2407(prevHeaderLine)+os.EOL), writeCB);
        return(line);
    }
    ...
    //if the line start with a space and we have previously seen a header, we have header folding
        ...
        return (prevHeaderLine+line.slice(1));
    }
